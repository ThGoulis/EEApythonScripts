...
def Surface_water_bodies_QE1_Biological_quality_elements_status(conn, countryCode, cYear, working_directory):
    # https://tableau.discomap.eea.europa.eu/t/Wateronline/views/WISE_SOW_QualityElement/SWB_QualityElement?:embed=y&:showShareOptions=true&:display_count=no&:showVizHome=no
    with open(
            working_directory + 'NewDash.18.Surface_water_bodies_QE1_Biological_quality_elements_status2016.csv',
            'w+', newline='') as f:
        header = ["Country", "Code", "Ecological Status Or Potential Value", "Number", "Number(%)"]
        write = csv.writer((f))
        write.writerow(header)
        cur = conn.cursor()
        ElementStatus = ["1", "2", "3", "4", "5"]
        QualityElement = cur.execute('SELECT distinct qeCode '
                                     'from SOW_SWB_QualityElement  '
                                     'where cYear == ? '
                                     'and qeCode like "QE1%" '
                                     , (cYear,)).fetchall()
        ...
        for country in countryCode:
            for quality in QualityElement:
                for status in ElementStatus:
                    data = cur.execute('SELECT countryCode, qeCode, qeStatusOrPotentialValue, '
                                       'COUNT(qeStatusOrPotentialValue) as Number, '
                                       'ROUND(COUNT(qeStatusOrPotentialValue) * 100.0 / ( '
                                       'SELECT COUNT(qeStatusOrPotentialValue) '
                                       'from SOW_SWB_QualityElement '
                                       'where countryCode = ? and cYear == ? and qeCode = ? '
                                       'and qeStatusOrPotentialValue <> "Unknown" '
                                       'and qeStatusOrPotentialValue <> "Unpopulated" '
                                       'and naturalAWBHMWB <> "unknown" '
                                       'and surfaceWaterBodyCategory <> "unpopulated" '
                                       '), 0)'
                                       'from SOW_SWB_QualityElement '
                                       'where countryCode = ? and cYear == ? '
                                       ...
                                       'and naturalAWBHMWB <> "unknown" '
                                       'and surfaceWaterBodyCategory <> "unpopulated" '
                                       'and qeStatusOrPotentialValue = ? and qeCode = ? '

                                       , (country, cYear, *quality, country, cYear, status, *quality)).fetchall()
                    write.writerows(data)
...
def Surface_water_bodies_QE2_status(conn, countryCode, cYear, working_directory):
    # https://tableau.discomap.eea.europa.eu/t/Wateronline/views/WISE_SOW_QualityElement/SWB_QualityElement?:embed=y&:showShareOptions=true&:display_count=no&:showVizHome=no
    with open(
            working_directory + 'NewDash.18.Surface_water_bodies_QE2_status2016.csv',
            'w+', newline='') as f:
        header = ["Country", "Code", "Ecological Status Or Potential Value", "Number", "Number (%)"]
        write = csv.writer((f))
        write.writerow(header)
            ...
            for quality in QualityElement:
                for status in ElementStatus:
                    data = cur.execute('SELECT countryCode, qeCode, qeStatusOrPotentialValue, '
                                       ...
                                       'and naturalAWBHMWB <> "unknown" '
                                       'and surfaceWaterBodyCategory <> "unpopulated" '
                                       'AND qeStatusOrPotentialValue <> "None"'
                                       '), 0)'
                                       'from SOW_SWB_QualityElement '
                                       'where countryCode = ? and cYear == ? '
                                       'and qeStatusOrPotentialValue <> "Unknown" '
...
def Surface_water_bodies_QE3_status(conn, countryCode, cYear, working_directory):
    # https://tableau.discomap.eea.europa.eu/t/Wateronline/views/WISE_SOW_QualityElement/SWB_QualityElement?:embed=y&:showShareOptions=true&:display_count=no&:showVizHome=no
    with open(
            working_directory + 'NewDash.18.Surface_water_bodies_QE3_status2016.csv',
            'w+', newline='') as f:
        header = ["Country", "Code", "Ecological Status Or Potential Value", "Number", "Number(%)"]
        write = csv.writer((f))
        write.writerow(header)
...
def Surface_water_bodies_QE3_3_status(conn, countryCode, cYear, working_directory):
    # https://tableau.discomap.eea.europa.eu/t/Wateronline/views/WISE_SOW_QualityElement/SWB_QualityElement?:embed=y&:showShareOptions=true&:display_count=no&:showVizHome=no
    with open(
            working_directory + 'NewDash.18.Surface_water_bodies_QE3_3_status2016.csv',
            'w+', newline='') as f:
        header = ["Country", "Code", "Ecological Status Or Potential Value", "Number", "Number(%)"]
        write = csv.writer((f))
        write.writerow(header)
...
def Surface_water_bodies_QE_Status(conn, countryCode, cYear, working_directory):
    # https://tableau.discomap.eea.europa.eu/t/Wateronline/views/WISE_SOW_QualityElement/SWB_QualityElement?:embed=y&:showShareOptions=true&:display_count=no&:showVizHome=no
    with open(
            working_directory + 'NewDash.18.Surface_water_bodies_QE_status2016.csv',
            'w+', newline='') as f:
        header = ["Country", "Code", "High", "Good", "Moderate", "Poor", "Bad", "High (%)", "Good (%)", "Moderate (%)",
                  "Poor (%)", "Bad (%)"]
        write = csv.writer((f))
        write.writerow(header)
        cur = conn.cursor()
        ...
        for country in countryCode:
            for temp in QualityElement:
                quality = ','.join(temp)
                dropStatus1 = '''DROP TABLE IF EXISTS Status1;'''

                createStatus1 = '''CREATE TEMPORARY TABLE Status1 AS SELECT countryCode,
                                                         cYear,
                                                         qeCode,
                                                         CASE WHEN qeCode = "''' + quality + '''" AND 
                                                                   qeStatusOrPotentialValue = 1 THEN count(euSurfaceWaterBodyCode) END AS High
                                                    FROM SOW_SWB_QualityElement
                                                   WHERE countryCode = "''' + country + '''" AND 
                                                         cYear == 2016 AND 
                                                         qeStatusOrPotentialValue <> "Unknown" AND 
                                                         qeStatusOrPotentialValue <> "Unpopulated" AND 
                                                         naturalAWBHMWB <> "unknown" AND 
                                                         surfaceWaterBodyCategory <> "unpopulated" AND 
                                                         qeCode = "''' + quality + '''" AND 
                                                         qeStatusOrPotentialValue = 1;'''

                dropStatus2 = '''DROP TABLE IF EXISTS Status2;'''

                createStatus2 = '''CREATE TEMPORARY TABLE Status2 AS SELECT countryCode,
                                                         cYear,
                                                         qeCode,
                                                         CASE WHEN qeCode = "''' + quality + '''" AND 
                                                                   qeStatusOrPotentialValue = 2 THEN count(euSurfaceWaterBodyCode) END AS Good
                                                    FROM SOW_SWB_QualityElement
                                                   WHERE countryCode = "''' + country + '''" AND 
                                                         cYear == 2016 AND 
                                                         qeStatusOrPotentialValue <> "Unknown" AND 
                                                         ...
                                                         surfaceWaterBodyCategory <> "unpopulated" AND 
                                                         qeCode = "''' + quality + '''" AND 
                                                         qeStatusOrPotentialValue = 2;'''

                dropStatus3 = '''DROP TABLE IF EXISTS Status3;'''

                createStatus3 = '''CREATE TEMPORARY TABLE Status3 AS SELECT countryCode,
                                                         cYear,
                                                         qeCode,
                                                         CASE WHEN qeCode = "''' + quality + '''" AND 
                                                                   qeStatusOrPotentialValue = 3 THEN count(euSurfaceWaterBodyCode) END AS Moderate
                                                    FROM SOW_SWB_QualityElement
                                                   WHERE countryCode = "''' + country + '''" AND 
                                                         cYear == 2016 AND 
                                                         qeStatusOrPotentialValue <> "Unknown" AND 
                                                         ...
                                                         surfaceWaterBodyCategory <> "unpopulated" AND 
                                                         qeCode = "''' + quality + '''" AND 
                                                         qeStatusOrPotentialValue = 3;'''

                dropStatus4 = '''DROP TABLE IF EXISTS Status4;'''

                createStatus4 = '''CREATE TEMPORARY TABLE Status4 AS SELECT countryCode,
                                                         cYear,
                                                         qeCode,
                                                         CASE WHEN qeCode = "''' + quality + '''" AND 
                                                                   qeStatusOrPotentialValue = 4 THEN count(euSurfaceWaterBodyCode) END AS Poor
                                                    FROM SOW_SWB_QualityElement
                                                   WHERE countryCode = "''' + country + '''" AND 
                                                         cYear == 2016 AND 
                                                         qeStatusOrPotentialValue <> "Unknown" AND 
                                                         ...
                                                         surfaceWaterBodyCategory <> "unpopulated" AND 
                                                         qeCode = "''' + quality + '''" AND 
                                                         qeStatusOrPotentialValue = 4;'''

                dropStatus5 = '''DROP TABLE IF EXISTS Status5;'''

                createStatus5 = '''CREATE TEMPORARY TABLE Status5 AS SELECT countryCode,
                                                         cYear,
                                                         qeCode,
                                                         CASE WHEN qeCode = "''' + quality + '''" AND 
                                                                   qeStatusOrPotentialValue = 5 THEN count(euSurfaceWaterBodyCode) END AS Bad
                                                    FROM SOW_SWB_QualityElement
                                                   WHERE countryCode = "''' + country + '''" AND 
                                                         cYear == 2016 AND 
                                                         qeStatusOrPotentialValue <> "Unknown" AND 
                ...
                *             100.0 / (
                 ...
                 WHERE countryCode = "''' + country + '''" AND 
                       ...
                       naturalAWBHMWB <> "unknown" AND 
                       surfaceWaterBodyCategory <> "unpopulated" AND 
                       qeCode = "''' + quality + '''" group by qeCode;'''
